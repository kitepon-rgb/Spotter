...
This file is the canonical instruction source for every coding agent working in this repository.
`CLAUDE.md` is only the Claude Code import entry and must contain `@AGENTS.md`.

## Repository Agent Contract

This repository is Claude-first. Claude Code, Bell, Haiku, and the existing
hook-driven workflow remain first-class. Codex support is an adapter / execution
option around the existing agent-neutral core; do not fork detector or reporting
behavior into separate Claude and Codex implementations.

Before changing code, read this file and [docs/open-issues.md](docs/open-issues.md).
For Claude / Codex dual-support work, also read
[docs/02_spotter-claude-contract.md](docs/02_spotter-claude-contract.md). For
catalog / tool-db behavior, read [docs/01_catalog-design.md](docs/01_catalog-design.md).
Completed plans and historical design snapshots under `docs/archive/` are reference
material, not required reading for normal work.

Preserve existing Claude command names, hook behavior, prompt contracts, daemon IPC
contracts, report wording, and test fixtures unless the owner explicitly approves a
breaking change. Before changing dual-support behavior, document the current Claude
contract and add regression coverage for it.

### Critical recursion safety contract

Spotter has a known high-impact failure mode: recursive hook / daemon proliferation.
The historical incident is archived in `docs/archive/spotter-plan.md` section 18.
...
- Daemon lifecycle uses app-level heartbeat plus UserPromptSubmit auto-resurrect. Do
  not restore `process.ppid` parent-watch cleanup; VSCode native Claude Code can
  report a short-lived wrapper as the parent.

Do not weaken or bypass this guarantee when adding Codex support, sidecars, isolated
worktrees, or new hook paths. Replace a guard only with an equal or stronger
guarantee, document the failure path covered, and add regression coverage.

### Agent implementation rules

- Follow §0 below: no hidden fallback, no silent failure, and no provisional
...
- Verify exact file contents directly before editing; do not rely only on summaries
...
端末固有のnvm version pathは配布unitへ固定しない。
...
FP/FN 0で通過し、connector 20/20 fresh・p95 107.52ms。公開commit `7cbc3a1`のCIは6/6 green、
npm `latest`、tag / GitHub Release、このMacのregistry由来global installを1.4.20へ同期した。
このreleaseでは初期安全境界としてopt-in配布し、後続v1.4.21でdefault-onへ移行する。
